...
logger = logging.getLogger("agent_os.observability.logger")
...
def setup_agent_logger(agent_name: str) -> logging.Logger:
    ...
    Logs are isolated under the active workspace logs directory (~/.agentos/workspaces/<ws>/logs/<agent_name>.log).
    """
    workspace = Workspace.current()
    workspace.ensure_exists()
    
    agent_logger = logging.getLogger(f"agent_os.agents.{agent_name}")
