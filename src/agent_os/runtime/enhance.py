...
logger = logging.getLogger("agent_os.runtime.enhance")


class EnhancedAgentProxy:
    """
    Proxy wrapper that intercepts calls to an existing agent's methods,
    injecting AgentOS Memory and Observability capabilities.
    """
    def __init__(
        self, 
        agent_instance: Any, 
    ...
    def _inject_context(self, task_input: str, memories: list) -> str:
        ...
        return (
            f"[CONTEXT MEMORY] Here is relevant context retrieved from AgentOS store:\n"
            f"{context_snippet}\n\n"
...
def enhance(
    ...
    """
    Wrap any existing agent class instance with AgentOS Memory and Observability hooks.
