...
# CLI App Setup
app = typer.Typer(help="AgentOS: The runtime for AI agents.")
agent_app = typer.Typer(help="Manage and run AgentOS agents.")
...
def workspace_list():
    ...
    table = Table(title="AgentOS Workspaces", show_header=True, header_style="bold cyan")
...
def workspace_select(name: str = typer.Argument(..., help="Name of the workspace to select.")):
    ...
    if not ws.path.exists():
        console.print(f"[bold red]Error:[/bold red] Workspace '{name}' does not exist. Create it first using 'agentos workspace create {name}'.")
        raise typer.Exit(1)
...
def server_start(
    ...
    console.print(f"[bold green]Starting AgentOS server on {host}:{port}...[/bold green]")
    uvicorn.run("agent_os.server.app:create_app", factory=True, host=host, port=port, log_level="info")
...
def studio_start(
    ...
    """Start the AgentOS server backend for Studio."""
    console.print(Panel(
        Text(f"AgentOS Studio Backend is launching at http://127.0.0.1:{port}\nOpen the desktop Studio app to connect.", style="green"),
        title="[bold green]AgentOS Studio Backend[/bold green]",
        border_style="green"
    ))
    uvicorn.run("agent_os.server.app:create_app", factory=True, host="127.0.0.1", port=port, log_level="info")
