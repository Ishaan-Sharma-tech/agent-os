...
from agent_os.core.models import WorkspaceConfig
...
class Workspace:
    """
    Represents an isolated workspace inside AgentOS.
    Manages directories, SQLite databases, and workspace-specific configurations.
    """
    def __init__(self, name: str):
        self.name = name
        self.path = settings.agentos_dir / "workspaces" / name
        self.db_path = self.path / "agentos.db"
    ...
    def list_all(cls) -> list["Workspace"]:
        ...
        workspaces_dir = settings.agentos_dir / "workspaces"
